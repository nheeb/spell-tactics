...
func discard(spell: Spell):
	if combat.hand.has(spell):
		...
		combat.animation_queue.append(AnimationObject.new(combat.ui, "remove_card", [spell]))
		combat.animation_queue.append(AnimationObject.new(combat.ui, "deselect_card", []))
	else:
