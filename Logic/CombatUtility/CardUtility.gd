...
func discard(spell: Spell):
	if combat.hand.has(spell):
		...
		combat.animation_queue.append(AnimationObject.new(Game.combat_ui, "remove_card", [spell]))
	else:
...
func draw():
	if not combat.deck.is_empty():
		...
		combat.animation_queue.append(AnimationObject.new(Game.combat_ui, "add_card", [spell]))
